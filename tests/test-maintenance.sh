...
#!/usr/bin/env python3
import os, sys
print(os.environ.get("FAKE_${upper}_JSON", '{}'))
...
ap.add_argument("--inventory-json")
...
status = json.loads(os.environ.get(
    ...
    '"n_merged_partial": 0, "n_fixed": 0, "n_skipped": 0, "warnings": []}'))
if args.status_file:
...
echo "=== 9. Phase1③: vault_inventory.py失敗時はPhase2へ--inventory-jsonを渡さず継続する ==="
{
  ...
  assert_eq "exit 0" "0" "$rc"
  assert_not_contains "--inventory-jsonは渡されない" "$(cat "$APPLY_ARGV_LOG")" "--inventory-json"
...
echo "=== 11. Phase1⑤: decision_propagation.pyのrc=1(波及漏れ検出)は正常扱い ==="
{
  ...
  RUN_DIR="$(latest_run_dir)"
  assert_contains "サマリ行に波及漏れ疑い件数が反映される" \
...
echo "=== 13. Phase2: maintenance_apply.pyがanomaly=trueを報告したらlast_success_atを更新せず通知する ==="
{
  ...
  LAST_STDOUT="$T/stdout.log"; LAST_STDERR="$T/stderr.log"
  rc=0
  FAKE_APPLY_STATUS_JSON='{"ok": false, "anomaly": true, "reason": "schema_violation: test", "n_promoted": 0, "n_merged": 0, "n_merged_partial": 0, "n_fixed": 0, "n_skipped": 0, "warnings": []}' \
    run_maintenance || rc=$?
...
echo "=== 15. Phase3: サマリ行に各件数(promote/merge/merge_partial/fix/skip)が反映される ==="
{
  T="$WORK_ROOT/t15"; mkdir -p "$T"
  setup_test_env "$T"
  LAST_STDOUT="$T/stdout.log"; LAST_STDERR="$T/stderr.log"
  rc=0
  FAKE_APPLY_STATUS_JSON='{"ok": true, "anomaly": false, "reason": null, "n_promoted": 2, "n_merged": 1, "n_merged_partial": 1, "n_fixed": 3, "n_skipped": 4, "warnings": []}' \
    run_maintenance || rc=$?
  assert_eq "exit 0" "0" "$rc"
  ...
  assert_contains "修正3件" "$FRAG_TEXT" "修正3件"
...
echo "=== 22. Phase2への--fragments-json/--inventory-json/--merge-jsonは全検出器成功時に全て渡される ==="
{
  ...
  assert_contains "--inventory-jsonが渡される" "$ARGV" "--inventory-json"
...
echo "=== 24. Phase3: Preferencesへの昇格があった夜はexport-public-vault.shが2回目も実行される ==="
{
  T="$WORK_ROOT/t24"; mkdir -p "$T"
  setup_test_env "$T"
  LAST_STDOUT="$T/stdout.log"; LAST_STDERR="$T/stderr.log"
  rc=0
  FAKE_APPLY_STATUS_JSON='{"ok": true, "anomaly": false, "reason": null, "n_promoted": 1, "n_merged": 0, "n_merged_partial": 0, "n_fixed": 0, "n_skipped": 0, "warnings": []}' \
    run_maintenance || rc=$?
  assert_eq "exit 0" "0" "$rc"
  # FAKE maintenance_apply.pyはapply-log.jsonを書かないため、本テストでは
  # RUN_DIR直下へ直接それらしいapply-log.jsonを事前に置けないので、
  # 代わりにexport呼び出し回数（Phase0で1回・Phase3の昇格検出で2回目）を
  # FAKE_EXPORT_CALL_LOGの行数で確認する。FAKE maintenance_apply.pyが
  # apply-log.jsonを書かない場合、HAD_PREFERENCES_PROMOTION判定は0になり
  # 2回目は呼ばれないはずなので、まずFAKE maintenance_apply.py改修無しでは
  # 2回目が呼ばれないことを確認し、次のテストで実際にapply-log.jsonへ
  # promote+Preferencesの結果を書かせて2回目が呼ばれることを確認する。
  calls_without_apply_log="$(wc -l < "$EXPORT_CALL_LOG" | tr -d ' ')"
  assert_eq "apply-log.json未生成時はexport再実行なし(Phase0の1回のみ)" "1" "$calls_without_apply_log"
}

echo "=== 25. Phase3: apply-log.jsonにPreferences昇格が記録されていればexport-public-vault.shが2回実行される ==="
{
  T="$WORK_ROOT/t25"; mkdir -p "$T"
  setup_test_env "$T"
  LAST_STDOUT="$T/stdout.log"; LAST_STDERR="$T/stderr.log"
  # FAKE maintenance_apply.pyはapply-log.jsonを書かない実装のため、本テストは
  # RUN_DIR確定後に直接apply-log.jsonを書き込む形で模擬する。maintenance.sh
  # 自体は一度先に走らせてRUN_DIRを確定させることができないため、代わりに
  # FAKE maintenance_apply.py自体をこのテストだけ差し替える。
  cat > "$REPO/scripts/vault-agents/maintenance_apply.py" <<'PYEOF'
#!/usr/bin/env python3
import argparse, json, os, sys
ap = argparse.ArgumentParser()
ap.add_argument("--vault"); ap.add_argument("--workdir", required=True); ap.add_argument("--status-file")
ap.add_argument("--claude-timeout"); ap.add_argument("--max-merge-actions")
ap.add_argument("--fragments-json"); ap.add_argument("--inventory-json"); ap.add_argument("--merge-json")
ap.add_argument("--dry-run", action="store_true")
args = ap.parse_args()
status = {"ok": True, "anomaly": False, "reason": None, "n_promoted": 1, "n_merged": 0,
          "n_merged_partial": 0, "n_fixed": 0, "n_skipped": 0, "warnings": []}
if args.status_file:
    with open(args.status_file, "w", encoding="utf-8") as f:
        json.dump(status, f)
log = {"date": "2026-07-16", "results": [
    {"id": "frag-x", "action": "promote", "applied": True, "note_path": "Preferences/new-note.md"}
], "warnings": []}
with open(os.path.join(args.workdir, "apply-log.json"), "w", encoding="utf-8") as f:
    json.dump(log, f)
sys.exit(0)
PYEOF
  chmod +x "$REPO/scripts/vault-agents/maintenance_apply.py"
  ...
  run_maintenance || rc=$?
  assert_eq "exit 0" "0" "$rc"
  calls="$(wc -l < "$EXPORT_CALL_LOG" | tr -d ' ')"
  assert_eq "export-public-vault.shがPhase0+Phase3の2回呼ばれる" "2" "$calls"
}
...
echo "=== 27. Phase2: apply-status.jsonのok/anomalyが矛盾する組合せ(ok=false かつ anomaly=false)は成功扱いにしない ==="
{
  ...
  LAST_STDOUT="$T/stdout.log"; LAST_STDERR="$T/stderr.log"
  rc=0
  FAKE_APPLY_STATUS_JSON='{"ok": false, "anomaly": false, "reason": null, "n_promoted": 0, "n_merged": 0, "n_merged_partial": 0, "n_fixed": 0, "n_skipped": 0, "warnings": []}' \
    run_maintenance || rc=$?
  assert_eq "exit 0" "0" "$rc"
...
echo "=== 30b. MAINTENANCE_LOG_ROOTのパスにシングルクォートが含まれても壊れず正常終了する（parse_step_status/apply-status.json/apply-log.json解析の回帰テスト・Codexレビュー指摘Minor対応） ==="
{
  T="$WORK_ROOT/t30b"; mkdir -p "$T"
  setup_test_env "$T"
  LAST_STDOUT="$T/stdout.log"; LAST_STDERR="$T/stderr.log"
  # LOG_ROOT自体をシングルクォートを含むパスへ差し替える。RUN_DIR（status-file
  # 群・apply-status.json・apply-log.jsonの置き場所）は全てLOG_ROOT配下のため、
  # parse_step_status()・Phase2のapply-status.json解析・HAD_PREFERENCES_
  # PROMOTION算出（apply-log.json解析）を一度に狙い撃ちで検証できる。
  # run_maintenance()自身が内部で`MAINTENANCE_LOG_ROOT="$LOG_ROOT"`という
  # 固定代入を行うため、外側から`MAINTENANCE_LOG_ROOT=...`を環境変数prefixで
  # 渡しても関数内側の代入に上書きされてしまう（このテストファイル冒頭の
  # 教訓と同型）。グローバル変数LOG_ROOT自体を書き換えるのが正しい。
  #
  # 標準のFAKE maintenance_apply.pyはapply-log.jsonを書かないため、それだけでは
  # HAD_PREFERENCES_PROMOTION算出（apply-log.json解析）の引用符回帰を検出できない
  # （Codexレビュー指摘Minor対応: 未修正のまま構文エラーになっても
  # `2>/dev/null || echo 0`でフォールバックし「Preferences昇格なし」と区別が
  # つかない）。テスト25と同じくapply-log.jsonを書くFAKE実装へ差し替え、
  # export-public-vault.shがPhase0+Phase3の2回呼ばれることまで確認する。
  cat > "$REPO/scripts/vault-agents/maintenance_apply.py" <<'PYEOF'
#!/usr/bin/env python3
import argparse, json, os, sys
ap = argparse.ArgumentParser()
...
PYEOF
  ...
  calls="$(wc -l < "$EXPORT_CALL_LOG" | tr -d ' ')"
  assert_eq "apply-log.json解析が構文破壊せず完走しPreferences昇格を検出・exportが2回呼ばれる" "2" "$calls"
}
...
" "$APPLY_STATUS")"
  assert_eq "実物のapply-status.jsonに7必須キーが全て揃っている(F2の実害範囲を直接検証)" "" "$MISSING_KEYS"
