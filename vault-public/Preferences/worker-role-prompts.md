...
| `verifier` | 検証 | 読んで指摘する・受入条件を実行して突合する・成果物には書かない・不足テストは指摘まで |
...
- repo（`claude/agents/`）に職種定義を足しただけでは `~/.claude/agents/` へ配布されない。`install-main.sh`／`install-sub.sh` は職種定義を1ファイルずつ symlink するため、新設時はインストーラの配置リストへの追加とセットで完了とする（`resolve` は OK でも spawn だけ失敗する型＝2026-09-03 と同型・2026-09-06 3モード設計レビューで再検出）。
