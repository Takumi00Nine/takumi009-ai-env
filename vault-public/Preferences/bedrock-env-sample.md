...
- 取り込まれるキー＝固定2つ（`CLAUDE_CODE_USE_BEDROCK`・`AWS_REGION`）＋動的（配役表の `role.*` に `provider=bedrock model=<別名>` の行があるときだけ、その別名に対応する `ANTHROPIC_DEFAULT_<別名大文字>_MODEL`）。使っていない別名のピン留め行は無視される。
