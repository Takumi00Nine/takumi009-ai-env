...
## 手順
1. **再開メモを書く**（リーダー直筆可＝`~/.claude` 配下）: `~/.claude/resume/YYYY-MM-DD-<slug>-resume.md`。内容＝状態（案件・工程・モード）／正本のパス／本人決定済み事項／次の工程の順番／継続用の識別子（Codex の thread_id 等）。
...
4. **旧セッションを閉じる**: 本人が閉じる（旧セッションから自分自身の `close-workspace` は未実測のため行わない）。
