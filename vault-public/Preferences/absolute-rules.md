...
---
# 絶対厳守ルール一覧
どのアシスタント（委任先 Codex 含む）も例外なく守る、非交渉のルール。**Codex へ委任する全プロンプトでこのノートを必読**（`mcp__codex__codex`/`codex-reply` の PreToolUse フックで強制＝参照が無いと拒否。**初回だけでなく継続往復の `codex-reply` 各送信にも参照を含めること**＝2026-08-08 ワーカー実測）。詳細は各リンク先。
