...
---
# 絶対厳守ルール一覧
どのアシスタント（委任先 Codex 含む）も例外なく守る、非交渉のルール。**Codex へ委任する全プロンプトでこのノートを必読**（`~/work/takumi009-ai-env/scripts/codex-exec.sh` ラッパーが依頼文に参照が無ければ起動を拒否して強制＝2026-09-06 に MCP フックから移行（[[Decisions/2026-09-06-codex-mcp-retire]]）。**初回だけでなく継続（`--resume`）の各送信にも参照を含めること**）。詳細は各リンク先。
