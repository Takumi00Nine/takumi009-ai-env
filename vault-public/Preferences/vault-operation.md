...
- **Tasks 節**（任意）: `## Tasks` → `### <版名>` → `- [ ]`/`- [/]`（進行中）/`- [x]`。cmux Dock「Task」枠の表示元。工程の節目にリーダーが記録職へ更新を依頼する。`next:` が無いノートは先頭未完タスクを Project 枠が導出表示する（書き戻しなし）。「Task の N 番」解決・Dock の展開規則は下の「Tasks の書き方」節参照。
