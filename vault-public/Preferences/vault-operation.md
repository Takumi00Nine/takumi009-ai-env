...
- **Projects の frontmatter**: `status:` は4値のみ＝`active`/`paused`/`completed`/`closed`。**状態が動いたら `next:`（15文字以内）も更新**（cmux Dock「Project」枠の表示元。「Project の N 番」解決＝`~/work/takumi009-ai-env/cmux/cmux-next-model.sh --list`＝5 列 TSV＝番号・正式プロジェクト名・next 値・区分（稼働中／待ち／保留）・待ち日時）。Project 枠の見出しは `● 稼働中 (n)`／`⏸ 待ち (n)`／`⏸ 保留 (n)`。▶＋強調色の行＝フォーカス中のワークスペースの宣言先の案件（2 秒周期・未宣言なら ▶ なし＝[[Decisions/2026-09-22-project-frame-focus-arrow-v7]]）。
