...
- **待ち（wait_until）**: 日時まで待つ案件は `wait_until: YYYY-MM-DDTHH:MM`（ローカル時刻・`YYYY-MM-DD` 可＝その日の 00:00）を足す（`status: active` のまま。Dock の Project 枠が「待ち」に分け、過ぎたら自動で稼働中に戻る＝書き戻し不要。過ぎた値は次に触るとき消す。秒・タイムゾーン付き・暦に無い日は無効＝稼働中のまま）。詳細＝[[Decisions/2026-09-20-project-frame-wait-status-v5]]。
