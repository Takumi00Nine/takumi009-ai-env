...
- **Projects の frontmatter**: `status:` は4値のみ＝`active`/`paused`/`completed`/`closed`。**状態が動いたら `next:`（15文字以内の次アクション）も更新**（cmux Dock「Project」枠の表示元。本人の「Project の N 番」解決＝`cmux-next-watch.sh --list`）。
- **Tasks 節**（任意）: `## Tasks` → `### <版名>` → `- [ ]`/`- [/]`（進行中）/`- [x]`。cmux Dock「Task」枠の表示元。本人の「Task の N 番」解決＝`cmux-task-watch.sh --list`（番号は恒久IDではなくその瞬間の表示順。展開中の版の子行だけに付く。Dock 枠は「Task」）。工程の節目（要件/設計/実装/検証の完了・版の切替）にリーダーが記録職へ更新を依頼する（[[Decisions/2026-09-09-cmux-session-todo-operation]]）。`next:` が無いノートは Tasks 節の先頭未完タスクを Project 枠が導出表示する（書き戻しなし）。
## 書き方の鉄則
