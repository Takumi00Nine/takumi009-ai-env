...
- **綻びは読み時（気づいた時点）で直す**: 鮮度（`review_by` 超過・古い外部可変情報＝一次情報で再確認→`updated` 引き直し。内部の決定ノートは対象外）／リンク切れ（張り直す or 除去）／alias（欠落・汎用/短すぎ＝想起に効く語へ）。⚠️ 範囲限定機（`vault_scope` が `full` でない）では、Preferences から範囲外ノートへのリンクはリンク切れに数えない（private フォルダ内のリンク切れは従来どおり直す＝[[Preferences/core-workflow]] §4・[[Decisions/2026-09-07-out-of-scope-links-not-broken]]）。
