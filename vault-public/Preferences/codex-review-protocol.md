...
- **締めの全体構成レビュー・後戻りコスト高の判断**: 既定 sol ＋ `config: {"model_reasoning_effort": "xhigh"}`（xhigh は実測で通る）。
...
- **モデル**: Codex＝既定 sol ＋ **effort `xhigh`**（2026-07-20 更新＝「使用モデルと effort の指標」節。xhigh 実測可を確認済み。旧推奨 high は xhigh 不明時代の値）。**後戻りコスト高・大規模なものは別モデル（例: Fable 5）でも独立レビューして2者クロス**（2モデルが収束した指摘は信頼度が高い＝実証済み）。
...
**締めの必須ゲートはもう1つある**＝**リーダーの本番経路 end-to-end スモークテスト**（全体構成レビューが"読み"の最終ゲートなのに対し、こちらは"実走"の最終ゲート）。本番と異なる実行環境（launchd/cron・別 PATH/権限・実バイナリ版・実データ量）で動く成果物は、本番の起動器で通しの実走を最低1回して初めて「完了」と言える。手順・実証＝[[Preferences/coding-delegation]]「締めの必須ゲート②」。
