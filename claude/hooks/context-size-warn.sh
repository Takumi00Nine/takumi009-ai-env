...
# リーダーに「次の切れ目で自分の判断でセッションを区切れ」という警告をコンテキスト注入する。
# （2026-09-22 本人決定: 本人への「分割/compact の提案」は廃止＝リーダーが確認なしで区切る。
#   Decisions/2026-09-22-leader-decides-session-split・手順＝Preferences/session-handoff）
...
#   CTX_WARN_AT      初回警告しきい値（トークン。既定 300000＝2026-09-03 本人決定。
#                    200k は 2026-08-10 本人決定（120k 起点だと直近14日の62%が警告対象で
#                    過剰なため引き上げた値）だったが、200k 運用では切替頻度が高すぎたため
#                    300k へ再度引き上げ。再警告の増分（+50k）は据え置き）
...
print(
    ...
    "今のタスクの切れ目（成果物の完成・検証巡の完了・工程の区切り）が来たら、本人に確認せずリーダーの判断でセッションを区切ること"
    "（手順＝Preferences/session-handoff・本人へは新セッション起動後に1行報告）。"
    "作業の中断はしない。この警告への言及は区切る時のみでよい。"
)
PYEOF
exit 0
